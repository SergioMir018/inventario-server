...
case class Product(id: UUID, name: String, short_desc: String, desc: String, price: Float, photo: String, category_id: UUID, deleted: Boolean)
...
class ProductTable(tag: Tag) extends Table[Product](tag, Some("products"), "Product") {
  ...
  override def * : ProvenShape[Product] = (product_id, name, short_desc, desc, price, photo, category_id, deleted).mapTo[Product]
}
...
object ProductTable {
  ...
  def getAllProducts(implicit ec: ExecutionContext): Future[Seq[ProductSearchResponse]] = {
    val query = for {
      (product, category) <- productTable.filter(_.deleted === false) join categoryTable on (_.category_id === _.category_id)
  ...
  def deleteProductById(id: UUID)(implicit ec: ExecutionContext): Future[Int] = {
    ...
    val markProductAsDeletedAction = productTable.filter(_.product_id === id).map(_.deleted).update(true)

    DatabaseConnection.db.run(markProductAsDeletedAction)
  }
